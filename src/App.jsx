/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║          TAJ MAHAL LORIENT — SPA Complète v2.0                  ║
 * ║  Stack : React 18 + Tailwind CSS + Framer Motion                ║
 * ║  Design: Terracotta · Safran · Crème · Playfair Display         ║
 * ║  Modules:                                                        ║
 * ║    1. restaurantData  — données exhaustives du menu              ║
 * ║    2. sanitize()      — protection XSS                          ║
 * ║    3. CartContext     — panier persistant (localStorage)         ║
 * ║    4. SEO / JSON-LD   — référencement local Lorient             ║
 * ║    5. Navbar          — sticky + badge panier                   ║
 * ║    6. Hero            — section d'accroche                      ║
 * ║    7. StorySection    — histoire du restaurant                   ║
 * ║    8. MenuSection     — toutes catégories + tous plats          ║
 * ║    9. CartDrawer      — slide-over panier                       ║
 * ║   10. CheckoutPage    — tunnel one-page + Stripe placeholder    ║
 * ║   11. Footer          — infos légales + horaires                ║
 * ╚══════════════════════════════════════════════════════════════════╝
 */
...
/* ─────────────────────────────────────────────────────────────────
   ...
   Source : objet fourni par le client, aucun plat omis
───────────────────────────────────────────────────────────────── */
export const restaurantData = {
  nom: "Taj Mahal Lorient",
  tel: "+33297000000",           // ← À remplacer
...
// Génère un ID unique stable pour chaque plat (catégorie:nom)
const makeId = (cat, nom) => `${cat}::${nom}`;

// Enrichissement : ajoute id à chaque plat
...
───────────────────────────────────────────────────────────────── */
/**
 * Nettoie une chaîne de caractères pour prévenir XSS et injections.
 * Supprime les balises HTML et caractères dangereux.
 * @param {string} str - Valeur brute saisie par l'utilisateur
 * @param {object} opts - { maxLength: number }
...
export function sanitize(str, { maxLength = 200 } = {}) {
  ...
  return str
    ...
    .replace(/javascript:/gi, "")       // bloque javascript: URIs
    .replace(/on\w+=/gi, "")           // bloque attributs événements inline
    .trim();
}

/** Valide un numéro de téléphone français (format laxiste) */
...
export function CartProvider({ children }) {
  ...
  const deliveryFee = total > 0 && total < 25 ? 2.50 : 0;
...
───────────────────────────────────────────────────────────────── */
export const SEO = {
  title: "Taj Mahal Lorient — Restaurant Indien Authentique & Commande à Emporter",
  description:
    "Restaurant indien à Lorient : Tikka, Biryani, Naan au tandoor. Commandez en ligne en 3 clics — à emporter ou livraison. Ouvert 7j/7. ⭐ 4.7/5 sur 218 avis.",
  jsonLd: {
...
function MenuCard({ item }) {
  ...
  return (
    <motion.article
      ...
      className="relative flex flex-col bg-[#3d1a0e]/60 border border-[#a45c40]/30 rounded-2xl overflow-hidden
                 hover:border-[#f4bb44]/50 hover:shadow-lg hover:shadow-[#f4bb44]/5 transition-all duration-300 group"
    >
      {/* Badges */}
      ...
      <div className="flex flex-col flex-1 p-4 pt-9 gap-2">
        <div className="flex justify-between items-start gap-2">
          <h3 className="font-semibold text-[#f5f5dc] text-sm leading-snug flex-1" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
            {item.nom}
          </h3>
          <span className="text-[#f4bb44] font-bold text-sm shrink-0">{item.prix.toFixed(2)} €</span>
        </div>
        ...
        <div className="mt-auto pt-2">
          {inCart ? (
            ...
            <motion.button
              ...
              onClick={handleAdd}
              className="w-full py-2 rounded-xl bg-gradient-to-r from-[#a45c40] to-[#c4723a]
                         text-[#f5f5dc] font-semibold text-xs tracking-wide
                         hover:from-[#f4bb44] hover:to-[#e8a832] hover:text-[#3d1a0e]
...
function MenuSection() {
  ...
  // Scroll horizontal vers l'onglet actif sur mobile
  ...
  return (
    <section id="menu" className="py-16 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="text-[#f4bb44]/70 text-xs uppercase tracking-[0.4em] mb-2 font-sans">Notre carte complète</p>
          <h2 className="text-4xl md:text-5xl text-[#f5f5dc]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
            Saveurs d'exception
          </h2>
          <div className="w-20 h-px bg-gradient-to-r from-transparent via-[#f4bb44]/60 to-transparent mx-auto mt-4" />
        </motion.div>

        {/* Onglets catégories — sticky sous la navbar */}
        <div
          ref={tabsRef}
          className="sticky top-[64px] z-20 flex gap-2 overflow-x-auto pb-3 mb-8 scrollbar-none
                     bg-gradient-to-b from-[#2c0f06] via-[#2c0f06]/95 to-transparent pt-2 -mx-4 px-4"
        >
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.aside
            role="dialog" aria-label="Panier de commande" aria-modal="true"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
            transition={{ type: "spring", damping: 28, stiffness: 280 }}
            className="fixed right-0 top-0 h-full w-full max-w-[360px] z-50 flex flex-col
                       bg-[#2c0f06] border-l border-[#a45c40]/40 shadow-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#a45c40]/30">
              <div>
                <h2 className="text-[#f5f5dc] font-bold text-base" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                  Votre commande
                </h2>
                <p className="text-[#c4a882] text-xs">{count} article{count > 1 ? "s" : ""}</p>
              </div>
              <button
                onClick={onClose}
                aria-label="Fermer le panier"
                className="w-8 h-8 flex items-center justify-center rounded-lg text-[#c4a882]
                           hover:text-[#f5f5dc] hover:bg-[#a45c40]/20 transition text-lg"
              >✕</button>
            </div>
              ...
              <AnimatePresence>
                {cart.length === 0 ? (
                  ...
                  >
                    ...
                    <button onClick={onClose} className="text-xs text-[#f4bb44] underline">Voir le menu</button>
                  </motion.div>
                ) : cart.map((item) => (
                  <motion.div
                    key={item.id}
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-[#f5f5dc] text-xs font-semibold leading-snug mb-0.5 truncate"
                         style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>{item.nom}</p>
                      <p className="text-[#f4bb44] text-xs font-bold">{(item.prix * item.qty).toFixed(2)} €</p>
                    </div>
                    {/* Qty */}
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => dispatch({ type: "DEC", id: item.id })}
                        className="w-6 h-6 flex items-center justify-center rounded-lg bg-[#a45c40]/30
                                   text-[#c4a882] hover:bg-[#f4bb44]/20 hover:text-[#f4bb44] transition text-sm font-bold">−</button>
                      <span className="text-[#f5f5dc] text-xs font-bold w-5 text-center">{item.qty}</span>
                    ...
                    <button onClick={() => dispatch({ type: "REMOVE", id: item.id })}
                      ...
                      className="text-[#c4a882]/40 hover:text-red-400 transition text-xs shrink-0 mt-0.5">✕</button>
                  </motion.div>
                ))}
              </AnimatePresence>
            </div>
              ...
              <div className="p-4 border-t border-[#a45c40]/30 space-y-2.5">
                ...
                {total < 25 && (
                  <div>
                    <p className="text-xs text-[#c4a882] mb-1.5">
                      Encore <span className="text-[#f4bb44] font-bold">{(25 - total).toFixed(2)} €</span> pour la livraison offerte
                    </p>
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-xl font-bold text-sm tracking-wide
                             bg-gradient-to-r from-[#f4bb44] to-[#e8a832] text-[#3d1a0e]
...
/* ─────────────────────────────────────────────────────────────────
   SECTION 8 — CHECKOUT ONE-PAGE (tunnel anti-abandon)
   Stripe Elements prêt à brancher
───────────────────────────────────────────────────────────────── */

/**
 * STRIPE INTEGRATION STEPS :
 * 1. npm install @stripe/stripe-js @stripe/react-stripe-js
 * 2. Créer /api/create-payment-intent.js (Next.js API route) :
 *      const stripe = require('stripe')(process.env.STRIPE_SECRET_KEY);
 *      export default async function handler(req, res) {
 *        const intent = await stripe.paymentIntents.create({
 *          amount: Math.round(req.body.amount * 100),
 *          currency: 'eur',
 *          metadata: { order: JSON.stringify(req.body.items), customer: req.body.name }
 *        });
 ...
 *      //   <PaymentElement />   ← remplacer le placeholder ci-dessous
 *      // </Elements>
 */
function CheckoutPage({ onBack }) {
  ...
  const [form, setForm] = useState({ name: "", phone: "", heure: "", address: "", note: "" });

  // Sanitisation à chaque frappe
  ...
  const validate = () => {
    ...
    if (mode === "delivery" && form.address.length < 5) errs.address = "Adresse de livraison requise";
    setErrors(errs);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    /**
     * ICI : appel à votre API Next.js pour créer le PaymentIntent Stripe
     * ET insertion dans Supabase / Firebase :
     *
     ...
     * // → charger Stripe Elements avec clientSecret
     *
     * SUPABASE (gestion commandes temps réel côté admin) :
  ...
  if (step === 2) return (
    ...
    >
      ...
      <h2 className="text-2xl text-[#f5f5dc] font-bold mb-2" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
        Commande confirmée !
      </h2>
      ...
      <p className="text-[#f4bb44] font-bold text-lg mb-8">Total : {grandTotal.toFixed(2)} €</p>
      <p className="text-[#c4a882] text-sm mb-6">Un SMS de confirmation sera envoyé au {form.phone}.</p>
    ...
    <div className="min-h-screen bg-[#2c0f06] py-8 px-4">
      <div className="max-w-md mx-auto">
        <div className="flex items-center gap-3 mb-8">
          <button onClick={onBack} className="text-[#c4a882] hover:text-[#f4bb44] transition text-lg">←</button>
          <h1 className="text-[#f5f5dc] text-xl font-bold" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
        ...
        <div className="bg-[#3d1a0e]/60 border border-[#a45c40]/30 rounded-2xl p-4 mb-6">
          <p className="text-xs text-[#c4a882] uppercase tracking-wider mb-3">Récapitulatif</p>
          <div className="space-y-1.5 max-h-40 overflow-y-auto">
            {cart.map((i) => (
              <div key={i.id} className="flex justify-between text-xs">
                <span className="text-[#c4a882] truncate mr-2">{i.nom} × {i.qty}</span>
                <span className="text-[#f5f5dc] shrink-0">{(i.prix * i.qty).toFixed(2)} €</span>
              </div>
            ))}
          </div>
        ...
        {/* Mode : À emporter vs Livraison */}
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[{ k: "pickup", icon: "🏪", label: "À emporter" }, { k: "delivery", icon: "🛵", label: "Livraison" }].map((m) => (
            <button key={m.k} onClick={() => setMode(m.k)}
              className={`flex flex-col items-center gap-1.5 py-4 rounded-xl border transition-all ${
                mode === m.k ? "border-[#f4bb44] bg-[#f4bb44]/10 text-[#f4bb44]"
        ...
        <div className="space-y-4 mb-6">
          {[
            ...
            <div key={f.name}>
              <label className="block text-xs text-[#c4a882] mb-1.5 uppercase tracking-wider">{f.label}</label>
              <input
                name={f.name} type={f.type} placeholder={f.placeholder}
                value={form[f.name]} onChange={handleChange}
                autoComplete={f.name === "phone" ? "tel" : f.name === "name" ? "name" : "off"}
                className={`w-full bg-[#3d1a0e] border rounded-xl px-4 py-3 text-[#f5f5dc] text-sm
                           placeholder:text-[#6b4030] focus:outline-none transition
                           ${errors[f.name] ? "border-red-500" : "border-[#a45c40]/40 focus:border-[#f4bb44]/60"}`}
              />
          ...
          {mode === "delivery" && (
            <div>
              <label className="block text-xs text-[#c4a882] mb-1.5 uppercase tracking-wider">Adresse de livraison *</label>
              <input
                name="address" type="text" placeholder="12 Rue de la Paix, Lorient"
                value={form.address} onChange={handleChange} autoComplete="street-address"
                className={`w-full bg-[#3d1a0e] border rounded-xl px-4 py-3 text-[#f5f5dc] text-sm
                           placeholder:text-[#6b4030] focus:outline-none transition
                           ${errors.address ? "border-red-500" : "border-[#a45c40]/40 focus:border-[#f4bb44]/60"}`}
              />
          ...
          <div>
            <label className="block text-xs text-[#c4a882] mb-1.5 uppercase tracking-wider">Instructions spéciales</label>
            <textarea
              name="note" placeholder="Allergie, sans piment, etc."
              value={form.note} onChange={handleChange} rows={2}
              className="w-full bg-[#3d1a0e] border border-[#a45c40]/40 rounded-xl px-4 py-3 text-[#f5f5dc] text-sm
                         placeholder:text-[#6b4030] focus:outline-none focus:border-[#f4bb44]/60 transition resize-none"
            />
            <p className="text-xs text-[#6b4030] text-right mt-0.5">{form.note.length}/300</p>
          </div>
        </div>

        {/* Stripe Placeholder */}
        <div className="bg-[#3d1a0e]/60 border border-[#a45c40]/30 rounded-2xl p-4 mb-6">
          <p className="text-xs text-[#c4a882] uppercase tracking-wider mb-3">Paiement sécurisé</p>
          {/* ← Remplacez ce div par <Elements stripe={stripePromise} options={{ clientSecret }}><PaymentElement /></Elements> */}
          <div className="border border-dashed border-[#f4bb44]/20 rounded-xl p-5 text-center">
            <p className="text-[#f4bb44] text-sm font-semibold mb-1">🔒 Stripe Elements</p>
            <p className="text-[#c4a882] text-xs leading-relaxed">
              Intégrez <code className="bg-[#2c0f06] px-1 rounded text-[#f4bb44]">{'<PaymentElement />'}</code><br/>
              de <code className="bg-[#2c0f06] px-1 rounded text-[#f4bb44]">@stripe/react-stripe-js</code>
            </p>
          </div>
          <label className="flex items-center gap-3 mt-3 p-3 border border-[#a45c40]/20 rounded-xl cursor-pointer">
            <input type="radio" name="payment" defaultChecked className="accent-[#f4bb44]" />
            <span className="text-sm text-[#c4a882]">Payer en espèces à la livraison / au comptoir</span>
          </label>
        </div>
...
function Navbar({ onCartOpen }) {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-5 py-3.5 transition-all duration-300 ${
        scrolled ? "bg-[#2c0f06]/95 backdrop-blur-md shadow-lg shadow-black/20" : "bg-transparent"
      } border-b border-[#a45c40]/20`}
    >
      <div>
        <span className="text-[#f5f5dc] text-lg font-bold" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
          Taj Mahal
        </span>
        <span className="text-[#f4bb44]/70 text-xs ml-2 hidden sm:inline font-sans">· Lorient</span>
      </div>
      ...
      <nav className="hidden md:flex items-center gap-6 text-sm text-[#c4a882]">
        ...
        <a href={`tel:${restaurantData.tel}`} className="hover:text-[#f4bb44] transition">Réserver</a>
      </nav>

      <button
        onClick={onCartOpen}
        aria-label={`Ouvrir le panier (${count} articles)`}
        className="relative flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm
                   bg-gradient-to-r from-[#a45c40] to-[#c4723a] text-[#f5f5dc]
                   hover:from-[#f4bb44] hover:to-[#e8a832] hover:text-[#3d1a0e] transition-all duration-300"
      >
        🛒 Panier
        <AnimatePresence>
          {count > 0 && (
            <motion.span
              ...
              className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-[#f4bb44] text-[#3d1a0e]
    ...
    <section className="relative min-h-[95vh] flex flex-col items-center justify-center text-center px-4 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute inset-0 opacity-[0.04]"
          style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23f4bb44' fill-opacity='1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")` }}
        />
      ...
      >
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <span className="text-[#f4bb44]/70 text-xs uppercase tracking-[0.4em] font-sans">Restaurant Indien</span>
          <div className="w-12 h-px bg-gradient-to-l from-transparent to-[#f4bb44]/50" />
        </div>

        <h1 className="text-6xl sm:text-7xl md:text-8xl text-[#f5f5dc] mb-3 leading-none"
          style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
          Taj Mahal
        </h1>
        <p className="text-[#f4bb44]/80 text-sm tracking-widest mb-4 font-sans uppercase">Lorient · Morbihan</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <motion.button
            ...
            onClick={onOrder}
            className="px-8 py-4 rounded-xl font-bold text-base tracking-wide
                       bg-gradient-to-r from-[#f4bb44] to-[#e8a832] text-[#3d1a0e]
                       shadow-xl shadow-[#f4bb44]/20 hover:brightness-110 transition-all duration-300"
          >
            🛵 Commander à emporter
          </motion.button>
          <a href={`tel:${restaurantData.tel}`}
            className="px-8 py-4 rounded-xl font-semibold text-base
                       border border-[#a45c40]/60 text-[#f4bb44] hover:bg-[#a45c40]/20 transition-all duration-300">
            📞 Réserver une table
          </a>
        </div>
      ...
      >
        {restaurantData.histoire.map((item, i) => (
          <motion.div
            ...
            className="bg-[#3d1a0e]/40 border border-[#a45c40]/25 rounded-2xl p-6 text-center
                       hover:border-[#f4bb44]/30 transition-all duration-300"
          >
            <span className="text-4xl block mb-4">{item.emoji}</span>
            <h3 className="text-[#f5f5dc] font-bold text-lg mb-2" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
              {item.titre}
            </h3>
...
function Footer() {
  return (
    <footer className="border-t border-[#a45c40]/20 py-12 px-4 text-center">
      <p className="text-[#f5f5dc] font-bold text-xl mb-1" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
        Taj Mahal
      </p>
      <p className="text-[#c4a882] text-sm mb-1">{restaurantData.adresse}</p>
      <p className="text-[#c4a882] text-sm mb-1">
        <a href={`tel:${restaurantData.tel}`} className="hover:text-[#f4bb44] transition">{restaurantData.tel}</a>
      </p>
      <p className="text-[#c4a882] text-sm mb-6">Lun–Dim : 11h30–14h30 · 18h30–22h30</p>
      <div className="flex justify-center gap-4 text-xs text-[#6b4030]">
        <a href="/mentions-legales" className="hover:text-[#c4a882] transition">Mentions légales</a>
        <span>·</span>
        <a href="/cgv" className="hover:text-[#c4a882] transition">CGV</a>
        <span>·</span>
        <a href="/confidentialite" className="hover:text-[#c4a882] transition">Confidentialité</a>
      </div>
      <p className="text-[#4a2a1a] text-xs mt-4">© {new Date().getFullYear()} Taj Mahal Lorient</p>
    </footer>
  );
}
...
/* ─────────────────────────────────────────────────────────────────
   SECTION 13 — APP ROOT
   Wrapping avec <CartProvider> dans index.js/main.jsx
───────────────────────────────────────────────────────────────── */
export default function App() {
  ...
  if (page === "checkout") {
    return (
      <CartProvider>
        <CheckoutPage onBack={() => setPage("home")} />
      </CartProvider>
    );
  }
  ...
  return (
    <CartProvider>
      {/* JSON-LD SEO — à déplacer dans <Head> si Next.js */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(SEO.jsonLd) }}
      />

      {/* Police Playfair Display via Google Fonts */}
...
── index.jsx (point d'entrée CRA ou Vite) ──────────────────────
...
── .env.local (JAMAIS commité dans Git) ─────────────────────────
  NEXT_PUBLIC_STRIPE_PK=pk_live_VOTRE_CLE_PUBLIQUE
